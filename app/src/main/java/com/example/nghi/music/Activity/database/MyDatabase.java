...
public class MyDatabase {
    ...
    public boolean isExistedSongInTable(int songid, int playlistid) {
        open();
        Cursor cursor;
        String select = "SELECT * FROM " + TABLE_MUSIC + " WHERE " + COLUMN_MUSIC_ID + " = '" + songid + "' AND "+ COLUMN_MUSIC_PLAYLIST_ID +" = '"+ playlistid +"'";
        cursor = db.rawQuery(select, null);
        if (cursor != null && cursor.getCount() > 0) {
            close();
            return true;
